...
var reorderColumnsByLast = function (list) {
    return list.sort(function (col1, col2) {
        if (col1.is_last)
            return 1;
        else if (col2.is_last)
            return -1;
        return 0;
    });
};
...
var reorderCards = function (list, startIndex, endIndex) {
    var result = Array.from(list);
    var removed = result.splice(startIndex, 1)[0];
    result.splice(endIndex, 0, removed);
    return result;
};
var insertCard = function (list, newCard, index) {
    var result = Array.from(list);
    result.splice(index, 0, newCard);
    return result;
};
var removeCard = function (list, index) {
    var result = Array.from(list);
    result.splice(index, 1);
    return result;
};

var BoardContext = React__default.createContext({
    columns: [],
    refresh: 0,
    setColumns: function (columns) { return null; }
});
var BoardContextProvider = /** @class */ (function (_super) {
    __extends(BoardContextProvider, _super);
    ...
    BoardContextProvider.prototype.componentDidUpdate = function (prevProps, prevState, snapshot) {
        if (!prevProps)
            return;
        var changed;
        switch (true) {
    ...
    BoardContextProvider.prototype.render = function () {
        return (React__default.createElement(BoardContext.Provider, { value: this.state }, this.props.children));
    };
    BoardContextProvider.prototype.setColumns = function (columns) {
...
function KbColumn(props) {
    ...
    var bc = React.useContext(BoardContext);
    var type = React.useState("CARD" +
        (bc.canChangeCardColumn ? (_.type ? "-" + _.type : "") : "-" + _.id))[0];
    return (React__default.createElement("div", { className: "kb-col-body", style: {
        ...
        React__default.createElement("div", { className: "kb-col-scroll" },
            ...
            React__default.createElement("div", { className: "kb-col-drop", style: { paddingTop: 0, marginTop: -6 } }, _.cards
                .filter(function (card) { return (card.is_last && card.locked); })
                .map(function (card, index) { return (React__default.createElement(KanbanCard, { _: card, i: index, key: card.id, has_ctxt: false })); })))));
}
...
function KbColumnDraggable(props) {
    ...
    return (React__default.createElement(React__default.Fragment, null,
            ...
            React__default.createElement(PublicDraggable, { draggableId: _.id, index: i, isDragDisabled: !!_.locked }, function (provided, snapshot) { return (React__default.createElement("div", __assign({ ref: provided.innerRef }, provided.draggableProps, provided.dragHandleProps, { className: "kb-col-header", style: __assign({ width: typeof columnWidth === "undefined" ? 240 : columnWidth }, getColumnHeaderStyle(snapshot.isDragging, provided.draggableProps.style)) }),
...
function KbBoard(props) {
    ...
    function onDragEnd(result) {
        ...
        var srcI = bc.columns.findIndex(function (col) { return col.id === source.droppableId; });
        ...
        if (result.type.startsWith("CARD")) {
            var newColumns = bc.columns;
            if (bc.canChangeCardColumn && srcI !== destI) {
                var insertedCard = newColumns[srcI].cards[source.index];
    ...
    return (React__default.createElement("div", { className: "kb-board" },
        React__default.createElement("div", { className: "kb-scroll" },
            React__default.createElement(DragDropContext, { onDragEnd: onDragEnd },
                React__default.createElement(ConnectedDroppable, { droppableId: "board", type: "COLUMN", direction: "horizontal", ignoreContainerClipping: false, isCombineEnabled: false }, function (provided, snapshot) { return (React__default.createElement("div", __assign({ ref: provided.innerRef, className: "kb-header", placeholder: "kb-header", style: getBoardHeaderStyle(snapshot.isDraggingOver) }, provided.droppableProps),
                    bc.columns
                        .map(function (column, index) { return (React__default.createElement(KbColumnDraggable, { _: column, key: column.id, i: index, columnWidth: columnWidth })); }),
                    provided.placeholder)); })),
            React__default.createElement("div", { className: "kb-body" },
                React__default.createElement(DragDropContext, { onDragEnd: onDragEnd }, bc.columns.map(function (column, index) { return React__default.createElement(KbColumn, { _: column, key: column.id, i: index, columnWidth: columnWidth }); }))))));
}

___$insertStyle(".kb-card {\n  width: 100%;\n  box-sizing: border-box;\n  user-select: none;\n  padding: 3px;\n  margin: 0;\n}");

var getCardStyle = function (isDragging, draggableStyle) { return (__assign({}, draggableStyle)); };
function KanbanCard(props) {
    var i = props.i, _ = props._;
    var bc = React.useContext(BoardContext);
    var inDrop = props.has_ctxt !== false;
    return (React__default.createElement(React__default.Fragment, null,
        inDrop &&
            React__default.createElement(PublicDraggable, { draggableId: _.id, index: i, isDragDisabled: !!_.locked }, function (provided, snapshot) { return (React__default.createElement("div", __assign({ ref: provided.innerRef }, provided.draggableProps, provided.dragHandleProps, { className: "kb-card", style: getCardStyle(snapshot.isDragging, provided.draggableProps.style) }), _.elem ? _.elem : React__default.createElement("div", null, "You must put a \"header\" element"))); }),
        !inDrop &&
            React__default.createElement("div", { className: "kb-card" }, _.elem ? _.elem : React__default.createElement("div", null, "You must put a \"header\" element"))));
}
